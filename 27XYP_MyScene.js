...
// 绘制几何对象
// 参数为模视矩阵
Obj.prototype.draw = function(matMVP){
	...
	// 开始绘制
	gl.uniformMatrix4fv(program.u_MVPMatrix, false, 
		flatten(matMVP)); // 传MV矩阵
...
function getLocation(){
	...
	program.u_NormalMat= gl.getAttribLocation(program, "u_NormalMat");
	if(!program.u_NormalMat){
		console.log("获取uniform变量u_NormalMat失败！"); 
	}

	program.LightPositions= gl.getAttribLocation(program, "u_LightPosition");
...
function render() {
	...
	var matMVP = mult(matProj, mult(translate(0, -jumpY, 0), matCamera));	
	...
	/*绘制绕原点旋转的球*/
	mvpStack.push(matMVP); // 使得下面对球的变换不影响后面绘制的圆环
		// 调整南北极后先旋转再平移
	...
	/*绘制自转的圆环*/
	matMVP = mult(matMVP, translate(0.0, 0.1, 0.0));
